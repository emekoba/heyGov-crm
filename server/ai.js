...
export async function parseAction(userQuery) {
...
function extractKeywordsFromQuery(query) {
	...
	return words.filter((w) => !STOP_WORDS.includes(w) && w.length > 1);
}

function filterRelevantContacts(contacts, query) {
...
export async function answerQuery(query, contacts) {
